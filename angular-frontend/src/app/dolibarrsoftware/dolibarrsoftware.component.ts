...
@Component({
  selector: 'app-dolibarrsoftware',
  templateUrl: './dolibarrsoftware.component.html',
  styleUrls: ['./dolibarrsoftware.component.scss']
})
export class DolibarrsoftwareComponent implements OnInit {
  ...
  constructor(public meta: Meta, public title: Title, public translate: TranslateService, private contactService: ContactService, private router: Router, private callService: CallService) {
    this.meta.addTags([
      {name: 'description', content: this.translate.instant('SEOINFO.META.META_DOLIBARR')}
    ]);
    this.title.setTitle(this.translate.instant('SEOINFO.TITLE.TITLE_PAGINA_DOLIBARR'));
  }
   ...
   ngOnInit(): void 
   {
    this.translate.onLangChange.subscribe((event: LangChangeEvent) => 
    {
      ...
      this.meta.addTags([
       ...
       if(event.lang == 'ESP')
       {
         $('.bannerDolibarr img').attr('src', '../../assets/imatgesDolibarrSoftware/banner-software-dolibarr-esp.png');
         $('.primeraImatge').css('background-image', "url('../../assets/imatgesDolibarrSoftware/primera-imatge-dolibarr-esp.png')");

       }
       else if(event.lang == 'CAT')
       {
        $('.bannerDolibarr img').attr('src', '../../assets/imatgesDolibarrSoftware/banner-software-dolibarr.png');
        $('.primeraImatge').css('background-image', "url('../../assets/imatgesDolibarrSoftware/primera-imatge-dolibarr.png')");
       }
     });
    // this.titleService.setTitle('Dolibarr i suport dolibarr ERP & CRM al núvol | Girohosting');
    // this.metaService.updateTag({name: 'description', content: 'Software de gestió empresarial per tot tipus de sectors de negocis o empreses - suport dolibarr integral 360 il·limitat - DOLIBARR | GIROHOSTING'}, 'name="description"');
