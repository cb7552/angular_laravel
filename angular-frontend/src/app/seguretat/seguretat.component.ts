...
@Component({
  selector: 'app-seguretat',
  templateUrl: './seguretat.component.html',
  styleUrls: ['./seguretat.component.scss']
})
export class SeguretatComponent implements OnInit {
  ...
  constructor(public meta: Meta, public title: Title, public translate: TranslateService, private contactService: ContactService, private router: Router, private callService: CallService) {
    ...
    this.meta.addTags([
      {name: 'description', content: this.translate.instant('SEOINFO.META.META_CIBERSEGURETAT')}
    ]);
    this.title.setTitle(this.translate.instant('SEOINFO.TITLE.TITLE_PAGINA_CIBERSEGURETAT'));
  }
  ...
  ngOnInit(): void
   {
    ...
    // this.titleService.setTitle('Cibereguretat informàtica professional | Girohosting ');
    // this.metaService.updateTag({name: 'description', content: 'Ciberseguretat informàtica el teu abast i donar tranquil·litat el teu negoci o empresa - triple seguretat amb Comodo - Sitelock - Codeguard | GIROHOSTING'}, 'name="description"');
